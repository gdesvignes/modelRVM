...
@njit(fastmath=False)
def get_L(cube, nI, nQ, nU, xm, Im, Qm, Um, bins, epochs, have_EFAC, nEFAC, rcvr_lut, nrcvr, prate=None, inc=None):
        ...
        comp_off = np.deg2rad(cube[ipar]); ipar += 1
        comp_width = np.deg2rad(cube[ipar]); ipar += 1
        if comp_width<0:
            return -1e16, betas
        comp_amp = cube[ipar]; ipar += 1
        comp_PA = np.deg2rad(cube[ipar]); ipar += 1
        ...
        for ii in range(nfiles):
            ...
            ibin = np.searchsorted(xm[ii], phi0 + comp_off) # Get the bin value for center of the gaussian
            max_core_comp = Im[ii][ibin]
            core_comp = comp_amp * max_core_comp * np.exp(-1./(2*comp_width**2)*(xm[ii]-(phi0 + comp_off))**2)

            #
            pulse_width_bin = int(comp_width / (xm[ii][1]-xm[ii][0]))
            prof = Im[ii]
            prof_diff = prof - core_comp
            # If the core component is brighter than the total intensity profile, rule out the live point
            if np.any(prof_diff[ibin-pulse_width_bin:ibin+pulse_width_bin] < - 3 *nI[ii]):
                return -1e16, betas
            ...
            zeta = alpha + beta
            sin_al = np.sin(alpha)
            xp = xm[ii][bins[ii]]-phi0
            argx = np.cos(alpha)*np.sin(zeta) - sin_al*np.cos(zeta)*np.cos(xp)
            argy =  sin_al * np.sin(xp)

            weighted_psi00 = core_comp[bins[ii]]/prof[bins[ii]] * psi00 + prof_diff[bins[ii]]/prof[bins[ii]] * (psi00+comp_PA) 
            
            PA2 = 2*(-np.arctan(argy/argx) + weighted_psi00 + eta + PA_off)
            cos2PA = np.cos(PA2)
            sin2PA = np.sin(PA2)

            Q = Qm[ii][bins[ii]]; U = Um[ii][bins[ii]];
            
            L = (Q * cos2PA/nQ2 + U * sin2PA/nU2) / (cos2PA*cos2PA/nQ2 + sin2PA*sin2PA/nU2) * np.exp(1j*PA2)

            chi += np.sum((Q-np.real(L))**2 / nQ2 + (U-np.imag(L))**2 / nU2)
            logdet += len(Q) * (np.log(nQ2) + np.log(nU2))
            betas[ii] = beta
...
class Precessnest():
    ...
    def set_labels(self):
        ...
        if self.have_inc:
            ...
            self.inc = 132.8
            #self.inc = 47.1
        # Parameters for core component
        self.labels.extend(["comp_off"])
        self.labels.extend(["comp_width"])
        self.labels.extend(["comp_amp"])
        self.labels.extend(["comp_PA"])
    ...
    def Prior(self, cube):
        ...
        pcube[ipar] = GaussianPrior(-10,15) (cube[ipar]); ipar += 1 # Core component offset
        pcube[ipar] = GaussianPrior(5,3) (cube[ipar]); ipar += 1 # Core component width
        pcube[ipar] = LogUniformPrior(0.02,1) (cube[ipar]); ipar += 1 # Core component Amp
        pcube[ipar] = GaussianPrior(10,10) (cube[ipar]); ipar += 1 # Core component PA offset
    ...
    def LogLikelihood(self, cube):
        return get_L(cube, self.nI, self.nQ, self.nU, self.xm, self.Im, self.Qm, self.Um, self.bins, self.epochs, self.have_EFAC, self.nEFAC, self.rcvr_lut,  self.nrcvr, prate=self.prate, inc=self.inc)
...
if rank==0:
    print("Relativistic spin-precession analysis (Kramer & Wex 2009). Version CPUs fp64")
    print("Ndim = %d\n"%ndims)
    print("nEFAC = %d\n"%model.get_nEFAC())
