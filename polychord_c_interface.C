...
double default_loglikelihood(double* theta, int nDims, double* phi, int nDerived, void* context)
{
    return 0.0;
}

void default_prior(double* cube, double* theta, int nDims, void *context)
{
    for(int i=0;i<nDims;i++) theta[i] = cube[i]; 
}

void run_polychord( double (*loglikelihood)(double*,int,double*,int,void*), Settings S) 
{
